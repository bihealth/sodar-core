...
class TestSyncRemoteDataCreate(SyncRemoteDataTestBase):
    ...
    def _update_remote_data(self):
        """Update user and role data for remote creation"""
        self.remote_data['users'].update(
            {
                SOURCE_USER2_UUID: {
        ...
        self.remote_data['projects'][SOURCE_CATEGORY_UUID]['roles'].update(
            {
        ...
        self.remote_data['projects'][SOURCE_PROJECT_UUID]['roles'].update(
            {
    ...
    def test_create_assert_db(self):
        """Test sync for creation and assert database changes"""
    ...
    def test_create_assert_projects(self):
        """Test sync for creation and assert project changes"""
        self.assertEqual(Project.objects.all().count(), 0)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        self.assertEqual(Project.objects.all().count(), 2)
    ...
    def test_create_assert_project_roles(self):
        """Test sync for creation and assert project role changes"""
        self.assertEqual(RoleAssignment.objects.all().count(), 0)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        self.assertEqual(RoleAssignment.objects.all().count(), 8)
    ...
    def test_create_assert_remote_objs(self):
        """Test sync for creation and assert remote site and project"""
        self.assertEqual(RemoteProject.objects.all().count(), 0)
        self.assertEqual(RemoteSite.objects.all().count(), 1)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
    ...
    def test_create_assert_remote_peer_objs(self):
        """Test sync for creation and assert remote peer objects"""
        self.assertEqual(RemoteProject.objects.all().count(), 0)
        self.assertEqual(RemoteSite.objects.all().count(), 1)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
    ...
    def test_create_assert_app_settings(self):
        """Test sync for creation and assert app settings"""
        self.assertEqual(AppSetting.objects.count(), 0)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        self.assertEqual(AppSetting.objects.count(), 3)
    ...
    def test_create_assert_remote_data(self):
        """Test sync for creation and assert remote data changes"""
        self.assertEqual(AppSetting.objects.count(), 0)
        self._update_remote_data()
        ...
        self.assertEqual(self.remote_data, expected)
    ...
    def test_create_multiple(self):
        ...
        ex_data = deepcopy(self.remote_data)
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        ...
        ex_data['users'][SOURCE_USER_UUID]['status'] = 'created'
        ex_data['projects'][SOURCE_CATEGORY_UUID]['status'] = 'created'
        ex_data['projects'][SOURCE_CATEGORY_UUID]['roles'][
            SOURCE_CATEGORY_ROLE_UUID
        ]['status'] = 'created'
        ex_data['projects'][SOURCE_PROJECT_UUID]['status'] = 'created'
        ex_data['projects'][SOURCE_PROJECT_UUID]['roles'][
            SOURCE_PROJECT_ROLE_UUID
        ]['status'] = 'created'
        ex_data['projects'][new_project_uuid]['status'] = 'created'
        ex_data['projects'][new_project_uuid]['roles'][new_role_uuid][
            'status'
        ] = 'created'
        ex_data['app_settings'][SET_IP_RESTRICT_UUID]['status'] = 'created'
        ex_data['app_settings'][SET_IP_ALLOW_LIST_UUID]['status'] = 'created'
        ex_data['app_settings'][SET_STAR_UUID]['status'] = 'created'
        self.assertEqual(self.remote_data, ex_data)
    ...
    def test_create_local_owner(self):
        ...
        self.remote_data['users'][SOURCE_USER_UUID]['username'] = 'source_admin'
        self.remote_data['users'][SOURCE_USER_UUID]['groups'] = [
            SYSTEM_USER_GROUP
        ]
        self.remote_data['projects'][SOURCE_CATEGORY_UUID]['roles'][
            SOURCE_CATEGORY_ROLE_UUID
        ]['user'] = 'source_admin'
        self.remote_data['projects'][SOURCE_PROJECT_UUID]['roles'][
            SOURCE_PROJECT_ROLE_UUID
        ]['user'] = 'source_admin'
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
    ...
    def test_create_inherited(self):
        ...
        new_role_uuid = str(uuid.uuid4())
        self.remote_data['users'][str(new_user_uuid)] = {
    ...
    def test_create_no_access(self):
        ...
        self.assertEqual(expected, self.remote_data)
    ...
    def test_create_local_user(self):
        ...
        self.remote_data['users'][local_user_uuid] = {
    ...
    def test_create_settings_user(self):
        ...
        self.remote_data['app_settings'][global_set_uuid] = {
            'name': 'notify_email_project',
            'type': APP_SETTING_TYPE_BOOLEAN,
            'value': '0',
            'value_json': {},
        ...
        self.remote_data['app_settings'][local_set_uuid] = {
            'name': 'user_str_setting',
            'type': APP_SETTING_TYPE_STRING,
            'value': 'Local value',
            'value_json': {},
        ...
        # Assert sync data
        self.assertEqual(
            self.remote_data['app_settings'][global_set_uuid]['status'],
            'created',
        )
        self.assertEqual(
            self.remote_data['app_settings'][local_set_uuid]['status'],
            'created',
        )
    ...
    def test_create_user_add_email(self):
        ...
        self.remote_data['users'][SOURCE_USER_UUID]['additional_emails'] = [
            ADD_EMAIL
        ]
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
...
class TestSyncRemoteDataUpdate(
    ...
    def _update_remote_data(self):
        """Update default data for remote update"""
        # Add new user and contributor role to source project
        self.remote_data['users'][str(self.new_user_uuid)] = {
            'sodar_uuid': self.new_user_uuid,
    ...
    def setUp(self):
        ...
        # Set up IDs
        self.new_user_username = 'newuser@' + SOURCE_USER_DOMAIN
        self.new_user_uuid = str(uuid.uuid4())
        self.new_role_uuid = str(uuid.uuid4())
        # Update default data for all tests
        self.remote_data['projects'][SOURCE_CATEGORY_UUID]['status'] = 'updated'
        self.remote_data['projects'][SOURCE_PROJECT_UUID]['status'] = 'updated'
        self.remote_data['users'][SOURCE_USER_UUID]['status'] = 'updated'

    def test_update_assert_db(self):
        """Test sync for update and assert database changes"""
        self.assertEqual(Project.objects.all().count(), 2)
        self.assertEqual(RoleAssignment.objects.all().count(), 2)
        self.assertEqual(User.objects.all().count(), 2)
        self.assertEqual(RemoteProject.objects.all().count(), 3)
    ...
    def test_update_assert_projects(self):
        """Test sync for update and assert project changes"""
        self.assertEqual(Project.objects.all().count(), 2)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        self.assertEqual(Project.objects.all().count(), 2)
    ...
    def test_update_assert_roles(self):
        ...
        self.assertEqual(RoleAssignment.objects.all().count(), 3)
        self.assertEqual(User.objects.all().count(), 3)
        new_user = User.objects.get(username=self.new_user_username)
    ...
    def test_update_assert_remote_peer_objs(self):
        """Test sync for updating and assert remote peer objects"""
        self.assertEqual(RemoteProject.objects.all().count(), 3)
        self.assertEqual(RemoteSite.objects.all().count(), 2)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
    ...
    def test_update_assert_app_settings(self):
        """Test sync for updating and assert app settings"""
        self.assertEqual(AppSetting.objects.count(), 3)
        self._update_remote_data()
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        self.assertEqual(AppSetting.objects.count(), 3)
    ...
    def test_update_assert_remote_data(self):
        """Test sync for updating and assert remote data changes"""
        self._update_remote_data()
        expected = deepcopy(self.remote_data)
        self.remote_api.sync_remote_data(self.source_site, self.remote_data)
        expected['users'][SOURCE_USER_UUID]['status'] = 'updated'
        expected['users'][self.new_user_uuid]['status'] = 'created'
        expected['projects'][SOURCE_CATEGORY_UUID]['status'] = 'updated'
        expected['projects'][SOURCE_PROJECT_UUID]['status'] = 'updated'
        expected['projects'][SOURCE_PROJECT_UUID]['roles'][self.new_role_uuid][
            'status'
        ] = 'created'
    ...
    def test_update_user_uuid_local_allow(self):
        ...
        self.remote_data['users'][local_uuid_source] = {
    ...
    def test_update_settings_local(self):
        ...
        ex_data['users'][SOURCE_USER_UUID]['status'] = 'updated'
        ex_data['projects'][SOURCE_CATEGORY_UUID]['status'] = 'updated'
        ex_data['projects'][SOURCE_PROJECT_UUID]['status'] = 'updated'
        ex_data['app_settings'][SET_IP_RESTRICT_UUID]['status'] = 'skipped'
        ex_data['app_settings'][SET_IP_ALLOW_LIST_UUID]['status'] = 'skipped'
        ex_data['app_settings'][SET_STAR_UUID]['status'] = 'skipped'
        self.assertEqual(self.remote_data, ex_data)
    ...
    def test_update_settings_no_app(self):
        ...
        # Change projectroles app settings
        self.remote_data['app_settings'][setting_uuid] = {
            'name': setting_name,
            'type': APP_SETTING_TYPE_BOOLEAN,
            'value': False,
            'value_json': {},
    ...
    def test_update_revoke(self):
        ...
        self.assertNotIn(
            str(new_user.sodar_uuid), self.remote_data['users'].keys()
        )
    ...
    def test_delete_role(self):
        ...
        expected['app_settings'][SET_IP_RESTRICT_UUID]['status'] = 'skipped'
        expected['app_settings'][SET_IP_ALLOW_LIST_UUID]['status'] = 'skipped'
        expected['app_settings'][SET_STAR_UUID]['status'] = 'skipped'
        self.assertEqual(self.remote_data, expected)
    ...
    def test_update_no_changes(self):
        ...
        ex_data['app_settings'][SET_IP_ALLOW_LIST_UUID]['status'] = 'skipped'
        ex_data['app_settings'][SET_STAR_UUID]['status'] = 'skipped'
        self.assertEqual(ex_data, self.remote_data)
